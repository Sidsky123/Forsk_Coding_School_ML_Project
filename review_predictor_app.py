...
def load_csv():
    ...
    scrappedetsyReviews = pd.read_csv("reviews_from_etsy.csv", index_col=False, skiprows=[0], names=["SNo", "Review"])
    scrappedetsyReviews['sentiment'] = [pred_review(x)[0] for x in scrappedetsyReviews['Review']]

"""
Predicting the sentiment of the review using the model
"""
...
"""
Collecting the string for wordcloud
"""
...
"""
Creating the wordcloud and returning it in an image format
"""
...
Structure:
              ...
              Heading
  Navbar to navigate contents       dropdown
  --------------------------------------
  |        Pie chart                    |
  --------------------------------------
       ---------------------------------------
...
def app_ui():
    main_layout=html.Div(
        ...
        style={'background-color': '#184D47'}
    )
    return main_layout

"""
Click event listeners for the evaluators
"""
